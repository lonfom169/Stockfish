...
namespace {
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
                        Move* quietsSearched, int* quietsFail, int quietCount, Move* capturesSearched, int* capturesFail,
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    int capturesFail[32], quietsFail[64];
    StateInfo st;
    ASSERT_ALIGNED(&st, Eval::NNUE::CacheLineSize);
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      if (move != bestMove)
      {
          if (captureOrPromotion && captureCount < 32)
          {
              ...
              capturesFail[captureCount] = value - alpha;
              captureCount++;
          }
          ...
          else if (!captureOrPromotion && quietCount < 64)
          {
              ...
              quietsFail[quietCount] = value - alpha;
              quietCount++;
          }
      }
    }
    ...
    */
    ...
    // If there is a move which produces search value greater than alpha we update stats of searched moves
        ...
        update_all_stats(pos, ss, bestMove, bestValue, beta, prevSq,
                         quietsSearched, quietsFail, quietCount, capturesSearched, capturesFail, captureCount, depth);
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    if (!pos.capture_or_promotion(bestMove))
    {
        ...
        {
            int quietPenalty = -bonus2 + 2 * quietsFail[i];
            thisThread->mainHistory[us][from_to(quietsSearched[i])] << quietPenalty;
            update_continuation_histories(ss, pos.moved_piece(quietsSearched[i]), to_sq(quietsSearched[i]), quietPenalty);
        }
    }
    else
    ...
    for (int i = 0; i < captureCount; ++i)
    {
        ...
        int capturePenalty = -bonus1 + 2 * capturesFail[i];
        captureHistory[moved_piece][to_sq(capturesSearched[i])][captured] << capturePenalty;
    }
  }
