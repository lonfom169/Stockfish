...
namespace {
  ...
  int stat_bonus(Depth d, Value fail) {
    return std::min((12 * d + 282 + fail / 4) * d - 349 , 1594);
  }
        ...
        if (ttMove)
        {
            if (ttValue >= beta)
            {
                ...
                if ((ss-1)->moveCount <= 2 && !priorCapture)
                    update_continuation_histories(ss-1, pos.piece_on(prevSq), prevSq, -stat_bonus(depth + 1, ttValue - beta));
            }
            // Penalty for a quiet ttMove that fails low (~1 Elo)
            else if (!ttCapture)
            {
                int penalty = -stat_bonus(depth, beta - ttValue);
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          if (value > alpha && d < newDepth)
          {
              ...
              int bonus = value > alpha ?  stat_bonus(newDepth, value - alpha)
                                        : -stat_bonus(newDepth, alpha - value);
    ...
    */
             ...
             && !priorCapture)
    {
        ...
        update_continuation_histories(ss-1, pos.piece_on(prevSq), prevSq, stat_bonus(depth, alpha - bestValue) * (1 + extraBonus));
    }
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    int bonus1 = stat_bonus(depth + 1, bestValue - beta);

    if (!pos.capture(bestMove))
    {
        int bonus2 = bestValue > beta + 137 ? bonus1               // larger bonus
                                            : stat_bonus(depth, bestValue - beta);   // smaller bonus
