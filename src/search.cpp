...
namespace {
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Move prevBestMove, Value bestValue, Value beta,
                        Square prevSq, Move* quietsSearched, int quietCount, Move* capturesSearched, int captureCount,
                        Depth depth);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    (ss+1)->ttPv         = false;
    (ss+1)->excludedMove = bestMove = prevBestMove = MOVE_NONE;
...
moves_loop: // When in check, search starts here
    ...
    */
    ...
    // If there is a move which produces search value greater than alpha we update stats of searched moves
        ...
        update_all_stats(pos, ss, bestMove, prevBestMove, bestValue, beta, prevSq,
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Move prevBestMove, Value bestValue, Value beta,
    ...
    if (prevBestMove)
    {
        if (!pos.capture(prevBestMove))
        {
            thisThread->mainHistory[us][from_to(prevBestMove)] << stat_bonus(depth);
            update_continuation_histories(ss, pos.moved_piece(prevBestMove), to_sq(prevBestMove), stat_bonus(depth));
        }
        else
            captureHistory[pos.moved_piece(prevBestMove)][to_sq(prevBestMove)][type_of(pos.piece_on(to_sq(prevBestMove)))] << bonus1;
    }
